...
CONFIG = dict(
    ...
    # 검색 게이트
    score_threshold=0.24,       # 0.0=끔. 예: 0.25 → 최고 관련도가 그 미만이면 LLM 호출 없이 거부
    # LLM
    llm_type="hf",
    model_name="LGAI-EXAONE/EXAONE-3.5-7.8B-Instruct",
    load_in_4bit=True,   # VRAM 부족(8GB GPU 등)이면 --load_in_4bit 로 켜기
    trust_remote_code=True,  # EXAONE 등 커스텀 코드 모델이면 --trust_remote_code 로 켜기
    # 검색 / 프롬프트
    prompt_name="basic",
    top_k=3,
    ...
    run_name="result_RIH_test",                # 결과 파일 이름(확장자 제외). 비우면 설정값으로 자동 생성
    use_ragas=True,
