...
CONFIG = dict(
    ...
    # 저장소 / 임베딩
    store_type="faiss",
    embedding_name="upstage",   # 최신 Upstage 임베딩(팀 통일 권장)
    embedding_model="",        # 특정 임베딩 모델명(예: BAAI/bge-m3). 비우면 embedding_name 기본모델 사용
    store_dir="../stores",     # 벡터DB 저장 폴더 (재사용으로 재임베딩 방지)
    rebuild_store=False,       # True면 저장된 것 무시하고 새로 생성
    # 검색 게이트
    score_threshold=0.2,       # 0.0=끔. 예: 0.25 → 최고 관련도가 그 미만이면 LLM 호출 없이 거부
    # LLM
    llm_type="hf",
...
def get_or_build_store(args):
    ...
    """
    # 임베딩 모델이 지정되면 캐시 키에도 반영(ko-sbert 인덱스를 bge-m3로 조회하는 사고 방지)
    emb_key = args.embedding_name + (f"_{_safe(args.embedding_model)}"
                                     if args.embedding_model else "")
    key = (f"{_safe(Path(args.data_path).stem)}_c{args.chunk_size}"
           f"_o{args.overlap_size}_{args.store_type}_{emb_key}")
    persist_dir = str(Path(args.store_dir) / key)
    emb_model = args.embedding_model or None   # 빈 문자열이면 기본모델(None)
