...
팀 공용 evaluate.py를 수정하지 않고 실제 사용 출처 기반의 인용
Precision/Recall/F1, 인용 교정 여부와 질문 유형을 추가로 기록합니다.
...
def answer_body(text: str) -> str:
    """
    사용자 답변 본문과 프로그램이 덧붙인 '(검증된 근거: ...)'를 분리합니다.

    기존에는 출처에 들어 있는 '별표2', '제167조' 같은 숫자가 정답 키워드로
    계산되어 실제 본문이 틀려도 keyword_rate가 올라갈 수 있었습니다.
    """
    return re.sub(
        r"\n*\(검증된 근거:\s*.*?\)\s*$",
        "",
...
def evaluate(
    ...
    for r in rows:
        ...
        rec = {
            ...
            "ground_truth": gt,
            "answer": res["answer"],
            # 생성 성능은 검증 출처 꼬리표를 제외한 순수 답변 본문으로 채점합니다.
            "answer_body": answer_body(res["answer"]),
            # 후처리 전 모델 출력도 저장해 모델 오류와 프로그램 오류를 구분합니다.
            "raw_answer": res.get("raw_answer", res["answer"]),
            "latency": res["latency"],
            "contexts": res["contexts"],
            "sources": "; ".join(res["sources"]),
            "prompt_sources": "; ".join(res.get("prompt_sources", [])),
            ...
            # 생성 한도에서 잘린 답변과 자동 재생성 효과를 따로 확인합니다.
            "initial_truncation_detected": res.get(
                "initial_truncation_detected", False
            ),
            "generation_retried": res.get("generation_retried", False),
            "truncation_detected": res.get("truncation_detected", False),
            "retry_max_new_tokens": res.get("retry_max_new_tokens", ""),
            "initial_generated_tokens": res.get(
                "initial_generated_tokens", ""
            ),
            "generated_tokens": res.get("generated_tokens", ""),
            "initial_hit_token_limit": res.get(
                "initial_hit_token_limit", False
            ),
            "hit_token_limit": res.get("hit_token_limit", False),
            # v4에서는 잘림뿐 아니라 필수 수치·문장별 인용 누락도 한 번 재작성합니다.
            # 최초 재작성 사유와 최종 잔여 문제를 분리해 자동 보정 효과를 확인합니다.
            "quality_retry_reasons": "; ".join(
                res.get("quality_retry_reasons", [])
            ),
            "quality_issues_remaining": "; ".join(
                res.get("quality_issues_remaining", [])
            ),
            "top_score": res.get("top_score", ""),
        }
        rec["generation_retry_rate"] = (
            1.0 if res.get("generation_retried", False) else 0.0
        )
        rec["truncation_rate"] = (
            1.0 if res.get("truncation_detected", False) else 0.0
        )
        rec["quality_retry_rate"] = (
            1.0 if res.get("quality_retry_reasons", []) else 0.0
        )
        rec["quality_issue_remaining_rate"] = (
            1.0 if res.get("quality_issues_remaining", []) else 0.0
        )
        if use_keyword:
            kws = extract_keywords(gt)
            rec["keyword_rate"] = keyword_rate(rec["answer_body"], kws)
        ...
        # 검증 전 모델 자체 인용 점수도 남겨 후처리로 점수를 숨기지 않습니다.
    ...
    if use_bertscore:
        scores = compute_bertscore(
            [x["answer_body"] for x in records],
...
_METRIC_COLS = ["bertscore_p", "bertscore_r", "bertscore_f1",
                ...
                "generation_retry_rate", "truncation_rate",
                "quality_retry_rate", "quality_issue_remaining_rate",
...
def _save_results(records, out_csv, exp_config):
    ...
    base_cols = ["id", "유형", "난이도", "question", "ground_truth",
                 "answer", "answer_body", "raw_answer", "keywords", "sources",
                 "prompt_sources",
                 "model_used_sources", "used_sources", "citation_repaired",
                 "contexts", "status", "citation_status", "question_mode",
                 "max_new_tokens_used", "initial_truncation_detected",
                 "generation_retried", "truncation_detected",
                 "retry_max_new_tokens", "initial_generated_tokens",
                 "generated_tokens", "initial_hit_token_limit",
                 "hit_token_limit", "quality_retry_reasons",
                 "quality_issues_remaining", "top_score"]
