...
중요:
  - 평가 질문이나 references.csv의 정답 조문을 이 파일에 넣지 않습니다.
  - 질문 문구를 보고 특정 조문을 강제로 선택하지 않습니다.
  - 검색 결과와 문서 본문만 사용하여 답변과 인용을 생성합니다.
...
PROMPTS = {
    ...
    "strict": {
        "system": _SYSTEM + " 제공된 법령 근거에만 기반해 답하세요. "
                  ...
                  "추측하거나 지어내지 마세요. "
                  "질문이나 근거를 되풀이하는 서론·맺음말·일반론은 작성하지 마세요. "
                  "단순 질문은 결론부터 1~3문장으로 답하고, 복합 질문은 사용자가 "
                  "요구한 항목마다 1~2문장으로 답하세요. "
                  "필요한 답과 실제 사용한 근거 ID를 작성한 직후 답변을 종료하세요."
                  + _CITE_RULE,
        "user": "[근거]\n{context}\n\n[질문]\n{question}",
        "require_citation": True,
    },
...
def get_llm(llm_type: str = "hf", model_name: str | None = None,
            temperature: float = 0.0, max_new_tokens: int = 180,
...
def _question_plan(
    question: str,
    single_max_tokens: int = 180,
    multi_max_tokens: int = 300,
) -> dict:
    """
    정답 조문을 모르는 상태에서 질문의 형식만 분석합니다.

    특정 설비명ㆍ직책명ㆍ수치ㆍ조문 번호는 사용하지 않습니다. 질문에 여러
    요구사항이 있는지만 판단하여 출력 길이와 일반적인 답변 형식을 정합니다.
    """
    ...
    if any(phrase in compact for phrase in ("무엇을기준", "어떤기준", "기준으로정해")):
        mode = "criteria"
        instruction = (
            "[기준 질문 출력 지침]\n"
            "- 첫 문장은 대상을 결정하는 기준 요소만 1문장으로 요약하세요.\n"
            "- 구체 기준이 별표에 있으면 검색 근거에서 확인되는 별표도 함께 쓰세요.\n"
            "- 질문하지 않은 세부 목록은 나열하지 마세요.\n"
            "- 실제 사용한 [C번호]를 붙이세요."
        )
        ...
        instruction = (
            ...
            "- 검색 근거 중 해당 의무를 직접 규정한 근거만 인용하세요.\n"
        ...
        # 접속 표현과 물음표 수만 사용해 복합질문 여부를 판단합니다.
        multi_markers = (
            "각각", "동시에", "비교", "단계별", "그리고", "뿐만아니라",
            "어떻게되고", "무엇이고", "어떤조치와", "및",
        )
        is_multi = (
            sum(compact.count(marker) for marker in multi_markers) > 0
            or question.count("?") > 1
        )
        ...
        if is_multi:
            instruction = (
                "[복합질문 출력 지침]\n"
                "- 질문에 포함된 요구사항을 순서대로 번호를 붙여 답하세요.\n"
                "- 항목당 1~2문장으로 작성하세요.\n"
                "- 각 항목 끝에 해당 내용을 직접 뒷받침하는 [C번호]만 붙이세요.\n"
                "- 근거에서 확인되지 않는 항목은 추측하지 말고 "
                "'근거에서 확인되지 않음'이라고 쓰세요."
            )

    return {
        "mode": mode,
        "max_new_tokens": int(
            multi_max_tokens if mode == "multi" else single_max_tokens
        ),
...
def _prepare_prompt_documents(question: str, docs: list, plan: dict) -> list:
    """
    검색 결과를 평가 정답과 무관한 방식으로 정리합니다.

    이전 버전은 질문 문구를 보고 특정 조문을 남기거나 제외했습니다. 그 방식은
    개발용 평가 문항에 과적합될 수 있으므로 제거합니다. 현재 버전은 검색 순서를
    보존하고 완전히 같은 출처만 중복 제거합니다.
    """
    prepared = []
    seen = set()
    for doc in docs:
...
def _citation_role_instruction(question: str, docs, plan: dict) -> str:
    """
    특정 조문을 지정하지 않고 인용 원칙만 안내합니다.

    어떤 C-ID가 정답인지는 모델이 문서 내용을 읽고 선택해야 합니다. 평가 문항별
    정답 조문을 프롬프트에 주입하지 않기 위해 문장 역할과 특정 C-ID를 연결하지
    않습니다.
    """
    return (
        "[인용 검증 원칙]\n"
        "- 각 문장에는 그 문장의 수치·의무·예외를 직접 뒷받침하는 C-ID만 붙이세요.\n"
        "- 법률, 시행령, 규칙의 역할을 추측하지 말고 제공된 문구를 직접 확인하세요.\n"
        "- 근거가 여러 개 필요한 문장은 실제로 사용한 C-ID를 모두 표시하세요."
    )
...
def _looks_truncated(
    ...
    return bool(
        re.search(
            r"(?:해야|대하여|관하여|따라|따르|또는|그리고|경우에|"
            r"이상인|초과하는|근로자가|사업주가)\s*$",
            tail,
        )
    )
...
def _quality_retry_issues(
    ...
    """
    정답 조문이나 정답 수치를 모르는 상태에서 형식적 품질만 검사합니다.

    특정 숫자ㆍ조문ㆍ설비명을 요구하지 않습니다. 빈 답변, 복합질문의 구조 누락,
    전체 인용 누락처럼 어떤 질문에도 공통으로 적용할 수 있는 문제만 찾습니다.
    """
    mode = plan.get("mode", "single")
    issues: list[str] = []

    cleaned = _clean_answer_artifacts(answer)
    compact_answer = re.sub(r"\s+", "", cleaned)
    if not compact_answer:
        return ["빈 답변"]
    if REFUSAL_MSG in answer:
        return []

    citation_ids = _extract_citation_ids(answer, docs)
    if docs and not citation_ids:
        issues.append("답변 전체에 C-ID 인용이 없음")

    if mode == "multi":
        # 특정 항목명을 알 필요 없이 번호 또는 여러 줄 구조가 있는지만 봅니다.
        numbered = re.findall(r"(?:^|\n)\s*(?:\d+[.)]|[-*•])\s+", answer)
        nonempty_lines = [line for line in answer.splitlines() if line.strip()]
        if len(numbered) < 2 and len(nonempty_lines) < 2:
            issues.append("복합질문의 요구사항을 구분한 항목 구조가 없음")
...
def _claim_support_score(claim: str, doc) -> float:
    ...
    # 질문/답변 단어와 조문 제목이 겹치면 약하게 우대합니다. 특정 법령명이나
    # 조문 번호를 열거하지 않으므로 새로운 질문에도 같은 방식으로 적용됩니다.
    title_tokens = set(
        _lexical_tokens(str(doc.metadata.get("조문제목", "")))
    )
    if claim_tokens and title_tokens:
        score += 0.2 * (
            len(claim_tokens & title_tokens) / max(len(title_tokens), 1)
        )
    return score


def _claim_role_source_match(claim: str, doc) -> bool:
    """정답 조문을 모르는 범용 버전에서는 역할별 강제 매칭을 사용하지 않습니다."""
    return False
...
def _lexical_tokens(text: str) -> list[str]:
    ...
    한국어에는 조사가 붙기 때문에 공백 단어만 비교하면 같은 명사에 조사가
    붙은 표현을 서로 다른 단어로 볼 수 있습니다.
...
def _expand_queries(question: str) -> list[str]:
    """
    복합질문을 문장 표현만으로 나누는 범용 질의 확장입니다.

    특정 직책ㆍ설비ㆍ법률 개념에 대한 정답 검색어를 추가하지 않습니다. 원 질문과
    사용자가 직접 작성한 하위 절만 검색하므로 평가 문항의 정답을 우회 주입하지
    않습니다.
    """
    q = re.sub(r"\s+", " ", question).strip()
    expanded = [q]

    # 쉼표ㆍ세미콜론ㆍ줄바꿈과 일반 접속 표현을 기준으로 하위 절을 분리합니다.
    clauses = re.split(
        r"[,;?\n]+|(?:\s+(?:그리고|또한|동시에|반면)\s+)",
        q,
    )
    original_key = re.sub(r"[^0-9A-Za-z가-힣]", "", q)
    for clause in clauses:
        clause = clause.strip(" .")
        clause_key = re.sub(r"[^0-9A-Za-z가-힣]", "", clause)
        if len(clause_key) >= 8 and clause_key != original_key:
...
def _scope_refusal(question: str) -> str | None:
    ...
    asks_latest_revision = (
        any(term in q for term in ("최신", "최근", "실시간", "오늘", "내일", "다음달", "내년"))
        and any(term in q for term in ("개정", "고시", "법령", "시행", "예정"))
    )
    ...
    individualized = any(
        term in q
        for term in ("우리현장", "현재현장", "우리회사", "제사고", "내사고")
    )
    asks_judgment = any(
        term in q
        for term in ("문제없", "가동해도", "돌려도", "책임자인지", "위법인지", "판단")
    )
    if individualized and asks_judgment:
        return (
...
class RagChain:
    def __init__(self, store, llm, prompt: dict,
                 ...
                 simple_max_new_tokens: int = 180,
                 multi_max_new_tokens: int = 300):
        self.store = store
    ...
    def _hybrid_retrieve(self, question: str):
        ...
        for query_index, query in enumerate(_expand_queries(question)):
            ...
            try:
                # relevance_scores API는 일부 FAISS/임베딩 조합에서 음수 점수를
                # 만들고 긴 경고문을 출력합니다. 하이브리드는 점수가 아니라 순위만
                # 사용하므로 raw score API로 같은 순서를 안전하게 가져옵니다.
                dense_pairs = self.store.similarity_search_with_score(
                    query, k=candidate_k
                )
            except Exception:
        ...
        for key, doc in docs_by_key.items():
            ...
            # 하위 절 검색의 1위 문서는 복합질문의 다른 근거일 수 있으므로 약하게
            # 우대합니다. 특정 설비명ㆍ직책명ㆍ조문 번호는 사용하지 않습니다.
            expansion_bonus = 0.1 if key in priority_set else 0.0
            combined = (
                0.45 * dense + 0.55 * lexical + expansion_bonus
            )
    ...
    def _retrieve_similarity(self, question: str):
        ...
        try:
            # raw FAISS 점수를 스토어가 제공하는 변환 함수로 0~1 범위에 맞춥니다.
            # 직접 relevance_scores API를 호출하지 않아 범위 경고도 발생하지 않습니다.
            raw_pairs = self.store.similarity_search_with_score(
                question, k=self.top_k
            )
            score_fn = self.store._select_relevance_score_fn()
            pairs = [
                (doc, max(0.0, min(1.0, float(score_fn(float(raw_score))))))
                for doc, raw_score in raw_pairs
            ]
            docs = [doc for doc, _ in pairs]
            top_score = max((score for _, score in pairs), default=0.0)
            return docs, round(float(top_score), 4)
        except Exception:
